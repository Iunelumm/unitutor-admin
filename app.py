...
def main():
    ...
    page = st.sidebar.radio(
        "导航",
        ["📊 平台统计", "👥 用户管理", "📅 会话管理", "⚠️ 争议处理", "💬 支持工单", "⭐ 评分管理", "👤 个人资料"]
    )
...
def show_users():
    ...
    col1, col2 = st.columns(2)
    with col1:
        role_filter = st.selectbox("角色筛选", ["全部", "普通用户", "管理员"])
    with col2:
        sort_by = st.selectbox("排序方式", ["最新注册", "最近登录", "姓名"])
    
    # 构建查询
    query = "SELECT id, name, email, role, loginMethod, preferredRoles, createdAt, lastSignedIn FROM users WHERE 1=1"
    params = []
...
def show_profiles():
    ...
    # 角色筛选
    role_filter = st.selectbox("角色筛选", ["全部", "学生", "教师"])
    ...
    if role_filter != "全部":
        role_map = {"学生": "student", "教师": "tutor"}
        query += " AND p.userRole = %s"
        params.append(role_map[role_filter])
    
    query += " ORDER BY p.createdAt DESC LIMIT 100"
    
    # 执行查询
    profiles = execute_query(query, params if params else None)
    ...
    if not profiles.empty:
        ...
        st.caption(f"显示 {len(profiles)} 个资料")
    else:
        st.info("没有找到资料")
