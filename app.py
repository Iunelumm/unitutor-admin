...
def show_user_detail(user_id):
    ...
    if not student_sessions.empty:
        ...
        col1.metric("学生会话总数", int(s['total']))
        col2.metric("已完成", int(s['completed']))
        col3.metric("已取消", int(s['cancelled']))
        col4.metric("有争议", int(s['disputed']))
    ...
    if not tutor_sessions.empty:
        ...
        col1.metric("教师会话总数", int(t['total']))
        col2.metric("已完成", int(t['completed']))
        col3.metric("已取消", int(t['cancelled']))
        col4.metric("有争议", int(t['disputed']))
